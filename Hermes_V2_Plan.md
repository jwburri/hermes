...
- **Phase D shipped 2026-09-10** (commit fe2c767): regression runner with a dry-run flag, refresh tick, login rate limit and constant-time compare, plus a guard that refuses to answer when Drive returned no documents (found the same day: the service account had lost its shared-drive membership and Hermes answered one question from prior answers alone). First full regression run pending the re-share. V2 is now complete apart from Aiman's review and the graded set.
- **Phase C shipped 2026-09-10** (commit eb0305c). Verified locally in production mode on Gronanda. A screenshot of a buyer email attached to a one-line instruction: Hermes read the "$15,000 net profit in May 2026" out of the image, corrected it to $7,667, and the checker (which sees the same attachment) flagged the premise. Six monthly COGS screenshots and a scanned PDF that were unreadable since June now count as read (56 files read, up from 49; about 200K cached tokens per call, up from 175K). Referred questions: a commission question and a supplier question were written to the Referred table as Open rows; the supplier one was resolved through the new screen, and the next answer used the reply verbatim with "Confirmed answers from the seller" in its coverage list, while the commission question was referred again. Dedupe is by wording, so a rephrased question creates a second Open row. Test rows deleted afterwards.
